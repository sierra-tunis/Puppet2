...
#ifndef PUPPET_GRAPHICS_HBOXGRAPHICS
#define PUPPET_GRAPHICS_HBOXGRAPHICS

class HboxGraphics : public Graphics<InternalObject> {
	...
	void drawObj(const InternalObject& obj,std::tuple<> cache) const override{
		...
		glUniformMatrix4fv(model_location, 1, GL_FALSE, obj.getGlobalPosition().data());
	...
	virtual std::tuple<> makeDataCache(const InternalObject& obj) const const {
		return std::tuple<>();
	}
